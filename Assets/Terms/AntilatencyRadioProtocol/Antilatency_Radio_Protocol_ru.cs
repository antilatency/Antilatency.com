...
partial class Terms {
    ...
    $"Устройства Antilatency передают данные через проприетарный радиопротокол, который работает на частоте 2.4ГГц. Радиопротокол оптимизирован для работы в режиме реального времени и имеет низкую задержку при передаче данных.")

        [$"В контексте радиопротокола {Terms.Socket} может выполнять роль клиента или точки доступа. Клиент-устройство передает точке доступа по радиопротоколу данные трекинга, полученные с модуля {Hardware.Alt}. Точка доступа собирает данные трекинга со всех подключённых клиентов, добавляет к ним свои и отправляет эту информацию на {Terms.Host} через USB."]
        ...
        [new Section("Несколько активных точек доступа в одном помещении")
            [$"Вы можете разместить несколько точек доступа в одном помещении, каждая из которых будет работать на отдельном радиоканале. Более того, они могут быть подключены к одному {Terms.Host}."]
            ...
            [new UnorderedList()
                [@$"*внешние факторы*
                    ...
                    Перед выбором радиоканалов мы рекомендуем просканировать диапазон 2.4 ГГц в помещении с помощью {Tutorials.RadioScan} или аналогичной программы.
