...
partial class Terms {
    ...
    $"Для передачи данных по радиоканалу в устройствах Antilatency используется проприетарный радиопротокол, работающий на частоте 2.4ГГц. Радиопротокол оптимизирован для работы в режиме реального и обладает низкой задержкой при передаче данных (low latency).")
        ...
        [new Section("Подключение передатчиков к конкретному приемнику: использование MasterSN and ChannelsMask","MasterSN")
            [@$"По умолчанию передатчики пытаются подключиться к ближайшему приемнику. Это поведение довольно простое и подходит для локального тестирования, когда в помещении работает только 1 приемник.
                Можно сконфигурировать передатчик, чтобы он подключался только к конкретному приемнику, используя свойство MasterSN. Получив Serial number приемника, его можно записать в свойство MasterSN передатчика, и теперь передатчик будет коннектиться только к приемнику с указанным Serial number.
                Кроме того, для ускорения подключения к приемнику передатчику можно задать маску каналов, по которой передатчик будет искать приемник для подключения.  
                Маска каналов (свойство ChannelsMask) - это строка длиной 141 символ (по количеству доступных каналов) , состоящая из “0” или “1”, где “1” означает, что соответствующий канал будет использован при поиске приемника, а “0” - что канал будет проигнорирован. 
                Если используется “полная маска” (строка, состоящая из всех “1”), поиск приемника может занять довольно длительное время. Но если ограничить поиск отдельными каналами, то подключение может занимать не более 100 миллисекунд.
                Маска по умолчанию для передатчиков выглядит следующим образом: 
                ...
                Положение символов “1” в данной маске соответствует списку каналов по умолчанию, используемых приемником. 
            "]
        ] 
