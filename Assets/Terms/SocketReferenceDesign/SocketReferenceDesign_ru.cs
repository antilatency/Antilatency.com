...
partial class Terms {
    ...
    @$"Отладочная плата для разработчиков {Terms.Socket}")
        ...
        [new Section("Рекомендации к плате")
            [new UnorderedList()
                [$"Height = 2mm для прессовки магнита."]
                [$"Покрытие - Immersion gold для качественного и долговечного контакта между погопинами {Hardware.Alt} и платой."]
            ]
        ]

        [new Section("Nrf52840 module")
            [$"Мы настойчиво рекомендуем использовать именно модуль с Nrf52840, а не ставить на свою плату контроллер Nrf52840 и обвязку для него. Использование модуля даёт такие преимущества:"]
            [new UnorderedList()
                ...
                [$"Все компоненты обвязки контроллера уже на своих местах - нет опасности расположить критичные компоненты не там."]
            ...
            [new Warning($"Отказ от использования модуля возможен. Это потребует расположения на своей плате контроллера, элементов обвязки и RF части. Однако необходимо понимать возможные последствия такого решения.")]
        ]
        ...
        [new Section($"{Hardware.Alt} connector")
            [$"Разъём для {Hardware.Alt} состоит из 8 сигнальных площадок и отверстия под магнит по центру. Рядом необходимо заложить монтажные отверстия для монтажа стенок, которые необходимы для выравнивания. См. {Tutorials.SocketReferenceDesignDeploying:mechanics} для лучшего понимания."]
        ]
        ...
        [new Section($"{Terms.Antilatency_Hardware_Extension_Interface} components")
            [$"Это опциональная часть. Если вы не планируете использовать {Terms.Antilatency_Hardware_Extension_Interface}, то соответсвующие компоненты можно не ставить, и пины контроллера никуда не подключать."]
            [$"На плате находятся 2 кнопки, 2 триммера-потенциометра и 4 светодиода. Каждый из этих компонентов подключен к контроллеру через джампер, с помощью него можно отключить компонент на плате и подключить что-то внешние."]
        ]
