...
partial class Software : Scope {
    static Material AntilatencyHardwareExtensionInterface_ru => new Material("Antilatency Hardware Extension Interface Library", null,
    $"Библиотека для {Terms.Antilatency_Hardware_Extension_Interface}")
        ...
        [$"*Todo info* До перехода таска в состояние *Run* все методы данных интерфейсов будут возвращать значение по умолчанию. "]

        [new Section("ICotask")
            [ICotaskCode]
                [$"*Todo info* Методы по созданию пинов только формируют таблицу инициализации, которая будет отправлена на устройство в методе `run()`."]
                ...
                [$"*Todo info* На текущий момент поддерживается только одинаковое время обновления для двух пинов в режиме счетчика импульсов."]
