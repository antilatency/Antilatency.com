...
partial class Tutorials : Scope {
    ...
    public static Material Alt_HMD_Placement_ru => new Material("Как создать Placement для HMD",
     TitleImage, 
     $"Один из способов настроить {Terms.Placement} для {Hardware.Alt} - с помощью вкладки {Software.AntilatencyService.Placements.Material} приложения {Software.AntilatencyService.Material}. Если среди предустановленных решений нет нужного вам устройства, вы можете создать свой Placement.")

    [$"Чтобы вручную задать Placement в AntilatencyService, нужно знать смещение и поворот трекера Alt относительно опорной точки физического объекта, на котором трекер будет установлен. Возьмём в качестве примера физического объекта шлем виртуальной реальности. Опорная точка HMD расположена между глазами пользователя. "]
