...
partial class Tutorials {
    ...
    [$"Все устройства Antilatency, в зависимости от их роли в системе {Terms.Antilatency_Radio_Protocol}, можно разделить на две группы: "]
            [new UnorderedList()
                ...
                [$"`точка доступа` - собирает данные с клиентов, добавляет к ним собственные данные и отправляет всё на {Terms.Host} через USB."]
            ]
      ...
      [new Section("Восстановление связи с клиентом")
       ...
       [new Section("Как сбросить свойство MasterSN", "ResetMaster") 
        ...
        [new Section($"Как назначить свойство MasterSN", "SetMasterHard")
                [new Section("C помощью AntilatencyService", "")]
                    [$"Для этого найдите во вкладке {Software.AntilatencyService.Device_Network.Material} нужную точку доступа и скопируйте её серийный номер."]
                        [accessPointSN]
                    [$"Чтобы установить MasterSN для клиента, следуйте этой инструкции: {Tutorials.Set_Device_Custom_Properties}"]
                
                [new Section("C помощью кнопки включения устройства", "")]
                [$"*Шаг 1.* Подключите точку доступа к {Terms.Host}."]
                [$"*Шаг 2.* Включите клиент-устройство коротким нажатием на кнопку включения."]
                [$"*Шаг 3.* Дождитесь, пока клиент подключится к точке доступа. Светодиоды обоих устройств будут равномерно моргать одним и тем же цветом."]
                [$"*Шаг 4.* Зажмите кнопку включения клиента на `5 секунд`. Устройство перезагрузится, а серийный номер точки доступа автоматически запишется в свойство MasterSN."]
