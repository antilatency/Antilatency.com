...
public partial class Tutorials : Scope {
    ...
    [$"HMD Radio Socket может работать как `клиент`, передавая данные трекинга с модуля {Hardware.Alt} точке доступа, или как `точка доступа`, собирая данные трекинга с подключённых клиентов и передавая их по USB на {Terms.Host} вместе с собственными данными."]
    ...
    [new Section("Отображение в Device Tree")
        ...
        [$"С версии прошивки 5.0.0 HMD Radio Socket в режиме `RadioSocket` отображается дополнительным узлом в {Terms.Device_Tree} под именем *AltHmdRadioSocketShadow* при подключении к {Terms.Host} через USB. Таким образом, можно настроить клиент, не подключая его к точке доступа."]
