...
public partial class Hardware : Scope {
    ...
    TopView,
    @$"Отладочная плата для разработчиков {Terms.Socket}, на основе которой можно собрать собственный Socket. Мы разработали общие рекомендации к параметрам самой платы и к дополнительным элементам, которые вы можете на неё поставить.")
        ...
        [new Section("SWD разъём","")
            [$"SWD разъём нужен для того, чтобы загрузить в первый раз bootloader в контроллер. После этого он больше не нужен, так как дальнейшая загрузка/обновление прошивки осуществляется через уже прошитый bootloader в {Terms.AntilatencyService}"]
