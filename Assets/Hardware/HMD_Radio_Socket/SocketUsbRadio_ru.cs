...
partial class Hardware : Scope {
    ...
    public static Material HMD_Radio_Socket_ru => new Material(
            ...
            HMD_Radio_Socket_Assets.HMD_Radio_SocketProduct0,
             $"Маленький и лёгкий {Terms.Socket}. {ThisDevice} может работать и как приёмник, и как передатчик. Более того, через {Hardware.ExtensionBoard} устройство может считывать внешние физические триггеры, например, нажание кнопки.")
                ...
                [new Section("Два режима работы")
                    [$"HMD Radio Socket может работать как приёмник или как передатчик. В режиме приёмника он собирает данные от других беспроводных устройств и передаёт их на {Terms.Host} через USB type-C. Если Socket подключён к внешней батарее, он может работать как передатчик, отправляя данные master устройству по радиопротоколу."]
                   // [new Info($"Подробнее о режимах работы читайте тут: {new ToDo("вставить ссылку, как будет отдельная статья", true)}")]
                ]
            ...
            [new Section("Техническая спецификация")
                [new Table(2)
                    [$"Интерфейс связи"][@$"2.4GHz проприетарный радиопротокол (режимы master и slave)
                                        USB 2.0 Full Speed"]
